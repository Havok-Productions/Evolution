...
final class TreeCanopyTransitionPolicy {
    ...
    static double progressiveCleanupCanopy(TreeDna dna) {
        return switch (dna.maturityStage()) {
            case SMALL -> 0.28D;
            case MEDIUM -> 0.32D;
            case MATURE -> 0.35D;
    ...
    static boolean shouldPeelStaleLayer(TreeDna dna, double canopyPercent,
            int consecutivePrunes) {
        // ## Below the full cleanup floor, alternate one stale leaf with one
        // replacement-canopy action so a former flat crown cannot linger.
        return canopyPercent >= progressiveCleanupCanopy(dna)
                && consecutivePrunes == 0;
    }
