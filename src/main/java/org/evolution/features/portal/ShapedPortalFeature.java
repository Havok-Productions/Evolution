...
public final class ShapedPortalFeature implements PluginFeature, Listener {
    private static final FeatureActionHierarchy<PortalActionPhase> ACTION_HIERARCHY =
            FeatureActionHierarchy.of("shaped-portals", PortalActionPhase.class);
    ...
    public ShapedPortalFeature(EvolutionPlugin plugin) {
        this.plugin = plugin;
    ...
    private void attemptCreate(World world, int x, int y, int z) {
        try (ReportSample sample = plugin.resourceReporter().begin(
            ...
            traceHierarchy(PortalActionPhase.DETECT_FRAME,
                    PortalActionSubrule.ENCLOSED_FRAME_FOUND,
                    "attempt-create");
            traceHierarchy(PortalActionPhase.DETECT_FRAME,
                    PortalActionSubrule.PROTECTION_ALLOWED,
                    "portal-protection");
            ...
            traceHierarchy(PortalActionPhase.CREATE_INTERIOR,
                    PortalActionSubrule.INTERIOR_COMMITTED,
                    "create-interior");
            ...
            TrackedPortal portal = new TrackedPortal(
                    ...
                    shape.interior(),
                    shape.frame());
            traceHierarchy(PortalActionPhase.TRACK_PORTAL,
                    PortalActionSubrule.EXACT_FOOTPRINT_INDEXED,
                    "track-created-portal");
            track(portal);
            savePortals();
    ...
    private void removePortal(TrackedPortal portal, String reason) {
        ...
        World sourceWorld = portal.world();
        if (sourceWorld != null && !portal.interior().isEmpty()) {
            PortalCell sourceCell = portal.interior().iterator().next();
            traceHierarchy(PortalActionPhase.REMOVE_PORTAL,
                    PortalActionSubrule.NETHER_SOURCE_RETIRED, reason);
            plugin.netherFeature().retirePortalSource(
                    sourceWorld.getBlockAt(sourceCell.x(),
                            sourceCell.y(), sourceCell.z()),
                    "shaped-portal-" + reason);
        }
        traceHierarchy(PortalActionPhase.REMOVE_PORTAL,
                PortalActionSubrule.FOOTPRINT_UNINDEXED, reason);
        unindex(portal);
        savePortals();
    ...
    private void clearPortalCells(World world, List<PortalCell> cells) {
        traceHierarchy(PortalActionPhase.REMOVE_PORTAL,
                PortalActionSubrule.INTERIOR_CLEARED,
                "clear-portal-interior");
    ...
    private synchronized void savePortals() {
        traceHierarchy(PortalActionPhase.PERSIST_PORTALS,
                PortalActionSubrule.PORTAL_STATE_SAVED,
                "save-portals");
    ...
    // ## SHAPED-PORTALS ACTION HIERARCHY
    private void traceHierarchy(
            PortalActionPhase phase,
            PortalActionSubrule subrule,
            String reason) {
