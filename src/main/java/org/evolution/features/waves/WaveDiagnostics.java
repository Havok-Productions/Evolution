...
final class WaveDiagnostics {
    ...
    private final AtomicLong regionPacketFallbackCells = new AtomicLong();
    ...
    // ## Viewer distribution is separate from static simulation and source counts.
    private final AtomicLong latestCoastAreas = new AtomicLong();
    private final AtomicLong latestLimitedCoastAreas = new AtomicLong();
    private final AtomicLong viewerSuppressedFronts = new AtomicLong();
    ...
    void recordRegionPacketFallback(int cells) {
        regionPacketFallbackCells.addAndGet(Math.max(0, cells));
    }
    ...
    void saveSoon(EvolutionPlugin plugin, WaveConfig config) {
    ...
    void saveAsync(EvolutionPlugin plugin, WaveConfig config) {
    ...
    void saveNow(EvolutionPlugin plugin, WaveConfig config) {
        plugin.pathDebug().trace(plugin, "waves", "persistence.save-debug.now", "waves.debug.yml");
        save(plugin, config);
    }

    private void save(EvolutionPlugin plugin, WaveConfig config) {
        ...
        yaml.set("counters.packet-batching.folia-direct-fallback-cells", regionPacketFallbackCells.get());
        ...
        yaml.set("counters.viewer-distribution.latest-coast-areas", latestCoastAreas.get());
        yaml.set("counters.viewer-distribution.latest-limited-coast-areas",
                latestLimitedCoastAreas.get());
        yaml.set("counters.viewer-distribution.suppressed-front-samples",
                viewerSuppressedFronts.get());
