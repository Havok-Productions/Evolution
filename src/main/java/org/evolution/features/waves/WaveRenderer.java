...
final class WaveRenderer {
    ...
    WaveRenderer(EvolutionPlugin plugin, WaveDiagnostics diagnostics) {
        this.plugin = plugin;
    ...
    private void sendWater(Player player, Map<WaveKey, Integer> locations, boolean packed) {
        ...
        // Cached wave columns may extend beyond the player's current Folia region. Those
        // cells must use direct packet data because taking a BlockState snapshot there is
        // an illegal cross-region world read.
        ...
        for (Map.Entry<WaveKey, Integer> entry : locations.entrySet()) {
            ...
            BlockData water = waterLevels[clampLevel(entry.getValue())];
            int chunkX = key.x() >> 4;
            int chunkZ = key.z() >> 4;
            boolean owned = Bukkit.isOwnedByCurrentRegion(world, chunkX, chunkZ, 0);
            if (owned && world.isChunkLoaded(chunkX, chunkZ)) {
                BlockState state = world.getBlockAt(key.x(), key.y(), key.z()).getState();
                state.setBlockData(water);
                states.add(state);
            } else {
        ...
        if (directFallbacks > 0) {
            diagnostics.recordRegionPacketFallback(directFallbacks);
            plugin.pathDebug().traceSampled(plugin, "waves", "render.region-packet-fallback",
                    "[SCHED][waves] direct-packet-cells=" + directFallbacks
                            + " ## cached visuals crossed the current Folia region; no world snapshot was read");
        }
    }
